...
\begin{enumerate}
    \item Présenter les fondements mathématiques de méthodes d'optimisation de portefeuille
...
\begin{figure}[H]
    \centering
    \fbox{
	\includegraphics[scale=0.2]{images/Equal_Weighted.png}
    }
    \caption{Allocation Equal-Weighted pour 6 actifs}
...
\begin{figure}[H]
    \centering
    \fbox{
	\includegraphics[scale=0.3]{images/EWP_IVP.png}
    }
...
\begin{figure}[H]
    \centering
    \fbox{
	\includegraphics[scale=0.3]{images/GVM.png}
    }
...
\begin{figure}[H]
    \centering
    \fbox{
	\includegraphics[scale=0.3]{images/MVO_GVM.png}
    }
...
\begin{figure}[H]
    \centering
    \fbox{
	\includegraphics[scale=0.4]{images/screenshot-app.png}
    }
...
\section{Performance Historique}
...
\begin{figure}[H]
    \centering
    \fbox{
	\includegraphics[scale=0.4]{images/dist-var.png}
    }
...
Ce projet a analysé l'optimisation de portefeuille, confirmant empiriquement que les méthodes simples et robustes, qui évitent l'estimation des rendements attendus ($\boldsymbol{\mu}$), surpassent souvent les modèles complexes en dehors de l'échantillon. L'utilisation du shrinkage de la covariance s'est avérée cruciale pour la stabilité des allocations. L'étude reste toutefois limitée par les hypothèses académiques classiques (normalité, stationnarité des rendements) et par la difficulté majeure d'estimer $\boldsymbol{\mu}$ avec précision.

Les perspectives pour surmonter ces défis incluent des approches théoriques comme l'optimisation robuste, les modèles factoriels et l'usage de mesures de risque alternatives (telles que la CVaR). Sur le plan pratique, les extensions futures se concentreront sur des backtests rigoureux incluant les coûts de transaction, l'intégration de contraintes réelles (sectorielles, ESG) et l'application de techniques de Machine Learning pour la prévision ou la construction de portefeuilles.

En conclusion, ce projet confirme qu'il n'existe pas de méthode universellement optimale ; la complexité théorique se heurte à l'incertitude pratique. Les leçons clés sont la supériorité fréquente de la simplicité en out-of-sample, l'importance cruciale de la robustesse (surtout vis-à-vis de $\boldsymbol{\mu}$), et la primauté de la diversification. Comme l'a souligné Harry Markowitz, celle-ci demeure le seul "free lunch" en finance.
...
Email : hassanelqadi3@gmail.com

GitHub : github.com/hassanelq

LinkedIn : linkedin.com/in/el-qadi
\end{center}
